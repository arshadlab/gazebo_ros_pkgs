  ...
  gazebo_msgs::srv::SetModelConfiguration::Response::SharedPtr _res)
{
  ...
  std::string gazebo_model_name = _req->model_name;
  ...
  if (_req->joint_names.size() == _req->joint_positions.size())
  {
    ...
    for (unsigned int i = 0; i < _req->joint_names.size(); i++)
    {
     ...
     "SetModelConfiguration: model called [%s] - %lf",_req->joint_names[i].c_str(), _req->joint_positions[i] );
    }
  ...
  {
    _res->success = false;
    _res->status_message = "set_joint_positions: joint name and position list have different lengths";
  }
